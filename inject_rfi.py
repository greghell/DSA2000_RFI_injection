...
def get_ms_data(msfile):
    ...
    * visidx : gives antennas indexes for each visibility
    ...
    t.close();
    visidx = np.zeros((len(ant1),2),dtype=int);visidx[:,0] = ant1;visidx[:,1] = ant2;
    t = table(msfile+'/ANTENNA');
    antspos = t.getcol('POSITION');
    t.close();
    antspos = EarthLocation.from_geocentric(x=antspos[:,0]*u.m,y=antspos[:,1]*u.m,z=antspos[:,2]*u.m).to_geodetic();
...
def FreeSpacePathLoss(LTEang, LTEdist, LTEheight, LTEfreq, antspos):
    ...
    for k in range(len(antspos)):
        d = np.sqrt((antspos[k,0] - LTE[0])**2 + (antspos[k,1] - LTE[1])**2 + (5 - LTE[2])**2); # 5 m altitude for each antenna
...
def SideLobesAtt(LTEang, LTEdist, LTEheight, LTEfreq, antspos, azel):
    ...
    az = np.deg2rad(azel[0].value+90);el = np.deg2rad(azel[1].value);
...
def calc_vis(FSPL, SLA, visidx, geodelays, trackdelays, t_acf, acf, polAngle, LTEpow):
    ...
    tot_att = FSPL[visidx[:,0]]*FSPL[visidx[:,1]]*SLA[visidx[:,0]]*SLA[visidx[:,1]];    # total attenuation path loss + side lobes
    ...
    t_acf = t_acf[delidx];acf = acf[delidx];
    t_acf = t_acf[::200];acf = acf[::200];
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog='inject_rfi.py',description='This program inject an RFI signal within an MS data set.')
    parser.add_argument('MSfile', help='MS file where the RFI will be injected in');
    parser.add_argument('--LTEdist', type=float, help='Distance between RFI transmitter and center of the telescope [m]',default=46800);
    parser.add_argument('--LTEang', type=float, help='Angle between RFI transmitter and center of the telescope [deg., 0=N, 90=W]',default=160);
    ...
    parser.add_argument('--LTEpol', type=float, help='Polarization angle of RFI [deg, 0=full XX, 90=full YY]',default=10);
    ...
    parser.add_argument('--OverWrite', action='store_true', help='Option to overwrite visibilities in input MS file as opposed to add them');
    args = parser.parse_args();

    msfile = args.MSfile;
    SITE_LOCATION = EarthLocation(lat=str(args.TelLat), lon=str(args.TelLon), height=1563); #generic for hot creek valley, can specify exact later
    ...
    LTEheight = float(args.LTEheight);  #height of aggressor antenna relative to height of array (assumes array zero reference)
    ...
    uvw, antspos, azel, visidx = get_ms_data(msfile);print('read MS file and extracted all relevant information.');
    FSPL = FreeSpacePathLoss(LTEang, LTEdist, LTEheight, LTEfreq, antspos);print('computed path loss between RFI Tx and telescops Rx');
    SLA = SideLobesAtt(LTEang, LTEdist, LTEheight, LTEfreq, antspos, azel);print('computed side lobe attenuation');
    geodelays = calc_geodelays(LTEang, LTEdist, LTEheight, antspos, visidx);print('computed geometric delays');
    trackdelays = calc_trackdelays(azel, antspos, visidx);print('computed tracking delays');
    rfi_acf = loadmat(rfi_acf);
    t_acf = rfi_acf['t_acf'][0];acf = rfi_acf['acf'][0];print('RFI correlation function loaded');
    print('computing visbilities...');
    vis = calc_vis(FSPL, SLA, visidx, geodelays, trackdelays, t_acf, acf, polAngle, LTEpow);print('Visibilities computed');
    vis_to_ms(vis, msfile, ow);print('Visibilities injected in '+msfile);
